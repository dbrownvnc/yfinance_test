        ...
        for ticker in tickers:
            if ticker and ticker not in existing_tickers:
                try: 
                    ...
                    # shortName을 우선으로 하고, 없으면 longName 사용
...
def get_realtime_news(ticker, name):
    ...
    # [Yahoo Finance RSS] - 티커 기준 (구조적 필요)
    if not is_kr:
        try:
    ...
    try:
        ...
        if mode == "10K":
            ...
            **모든 답변은 반드시 한글로 작성해 주십시오.**
            ...
            # [수정] 성장주/가치주 분석 + 투자성향별 비중 분석 로직 추가
            prompt = f"""
            [역할] 월스트리트 수석 애널리스트
            [대상] {ticker} (공식 기업명: {stock_name})
            [모드] {mode}
            ...
            **주의: '{ticker}'는 '{stock_name}'입니다. 다른 기업과 혼동하지 마십시오.**
            ...
            [분석 지침]
            1. **가장 먼저 이 기업이 '성장주(Growth Stock)'인지 '가치주(Value Stock)'인지 정의하십시오.**
            2. 그 성격에 따라 다음 항목을 **핵심적으로** 분석에 포함하십시오:

            **[성장주(Growth Stock)로 판단 시]**
            - **매출 성장률**: 최근 5년간 지속적인 상승 추세인지 여부.
            - **Cash Flow**: 영업 현금 흐름이 꾸준히 증가하고 있는지 분석.
            - **ROI (Return on Investment)**: 투자 대비 수익률의 개선 여부.
            - **Profit Margin 추이**: 마이너스에서 플러스로 전환되는 방향성 및 개선 속도.
            - **지속성**: 실적이나 지표가 급등락 없이 안정적으로 개선되고 있는지 평가.

            **[가치주(Value Stock)로 판단 시]**
            - **시장 점유율**: 증가 또는 감소 추세 확인 (감소 시 배당 축소 리스크 점검).
            - **배당금 안정성**: 꾸준한 지급 여부, 배당금 증액/삭감 추이 분석.
            - **주가 안정성**: 과거 데이터를 바탕으로 변동성이 낮은지 파악.
            - **이익률 변화**: Profit Margin의 상승 추세 (시장 지배력 및 경쟁력 강화 여부).
            - **EPS 변화**: 주당순이익의 장기적 트렌드 추적.
            
            3. **[투자성향별 포트폴리오 비중 분석]** (만약 '투자성향별 포트폴리오 적정보유비중'이 중점 분석 항목에 포함되어 있다면):
